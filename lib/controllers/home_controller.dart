...
class HomeController extends GetxController {
  ...
  final String testVideoLink =
      'https://server15700.contentdm.oclc.org/dmwebservices/index.php?q=dmGetStreamingFile/p15700coll2/15.mp4/byte/json';
  final String testSubtitleLink =
      'https://cdmdemo.contentdm.oclc.org/utils/getfile/collection/p15700coll2/id/18/filename/video2.vtt';
  ...
  void playM3U8VideoWithQualities() async {
    final content = await get(Uri.parse(
        "https://cph-p2p-msl.akamaized.net/hls/live/2000341/test/master.m3u8"));
    List<QualityClass> qualities = [];
    try {
      var hlsPlaylistParser = HlsPlaylistParser.create();
      var playlist =
          await hlsPlaylistParser.parseString(Uri.parse(''), content.body);
        ...
        for (Variant variant in variants) {
          qualities.add(
            QualityClass(
              ...
              link:
                  "https://cph-p2p-msl.akamaized.net/hls/live/2000341/test/${variant.url}",
            ),
          );
        }
  ...
  Future<void> playVideo(VideoClass video) async {
    ...
    if (isSubtitleEnabled.value == true &&
        video.subtitle != null &&
        video.subtitle![0].trim().isNotEmpty) {
      player.setSubtitleTrack(
          SubtitleTrack.uri(video.subtitle![0], language: "en"));
    } else {
      player.setSubtitleTrack(SubtitleTrack.no());
    }
  ...
  Future<void> pickMultipleVideos() async {
    ...
    try {
      pickedFiles = (await FilePicker.platform
                      ...
                      child: ListView(
                        ...
                        children: [0.5, 1.0, 1.5, 2.0].map((speed) {
                          return Padding(
                            ...
                            child: Container(
                              decoration: BoxDecoration(
                                  color: speed == playbackSpeed.value
                                      ? Colors.red
                                      : Colors.black45,
                                  borderRadius: const BorderRadius.all(
                                      Radius.circular(15))),
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: currentVideo != null && currentVideo?.qualities != null
                      ? ListView(
                          shrinkWrap: false,
                          physics: const NeverScrollableScrollPhysics(),
                          children: [
                            for (var i = 0;
                                i < currentVideo!.qualities!.length;
                                i++)
                              Padding(
                                padding: const EdgeInsets.only(top: 8.0),
                                child: Container(
